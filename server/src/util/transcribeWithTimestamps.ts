...
export const transcribeWithTimestamps = (
  ...
  response.results.forEach((result) => {
    result.alternatives?.[0]?.words?.forEach((wordInfo) => {
      // Ensure startTime is not null or undefined and is a valid string
      if (
        wordInfo.startTime?.seconds &&
        wordInfo.startTime?.nanos &&
        typeof wordInfo.word === "string"
      ) {
        // Calculate the start and end times in seconds
        const startSecs = parseFloat(
          `${wordInfo.startTime.seconds}` +
            "." +
            (wordInfo.startTime.nanos ?? 0) / 100000000
        );
